...
hl.on("hyprland.start", function()
 ...
 hl.exec_cmd("awww img /home/aadidev/Downloads/dino-wallpaper.jpg")
 hl.exec_cmd("vim.opt")
 hl.exec_cmd("mako")
end)
...
hl.animation({ leaf = "global",        enabled = true,  speed = 1.9,   bezier = "default" })
...
hl.animation({ leaf = "workspacesIn",  enabled = true, speed = 4, bezier = "quick", style = "slide" })
...
hl.bind(mainMod .. " + escape", hl.dsp.exec_cmd(
    "if pgrep -x wlogout >/dev/null; then pkill -x wlogout; else wlogout; fi"
))
...
hl.on("hyprland.start", function()
    hl.exec_cmd("systemctl --user import-environment WAYLAND_DISPLAY XDG_CURRENT_DESKTOP XDG_RUNTIME_DIR")
    hl.exec_cmd("dbus-update-activation-environment --systemd WAYLAND_DISPLAY XDG_CURRENT_DESKTOP XDG_RUNTIME_DIR")
